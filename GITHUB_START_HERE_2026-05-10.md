...
- `MCAContracts_Netlify_Deploy_LegalParity_2026-05-10_FINAL_VERIFIED_20260510a.zip`
  - Preferred deploy package after the final Declaration-page legal-content fix.
  - Cache-busts `js/app.js` with `v=20260510a`.
  - Rebuilds Declaration pages as full custom pages so extracted/legal text no longer contains the incomplete short seller phrase.

- `MCAContracts_Netlify_Deploy_LegalParity_2026-05-10.zip`
  - Prior legal-parity package.
...
- Final 2026-05-10a fix: Declaration of Ordinary Course pages now render as full custom pages at each funder’s page position (`35`, `88`, `142`) instead of relying on a partial overlay. This removes the live SMB page 42 defect where extracted text omitted `and JULIO VELAZQUEZ VILLALOBOS DBA JL&D Electric LLC`.
